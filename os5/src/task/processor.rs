...
pub fn run_tasks() {
    loop {
        ...
        if let Some(task) = fetch_task() {
            ...
            //update
            let mut stride = BIG_STRIDE / task_inner.prio;
            if stride == 0 {
                stride = 1;
            }
            task_inner.pass = task_inner.pass.wrapping_add(stride);
...
pub fn set_prority(proi: isize) {
    let task = current_task().unwrap();
    task.inner_exclusive_access().set_prority(proi);
}
